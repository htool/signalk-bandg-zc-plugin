...
3. Register the same defs as **`canboat-custom-pgns`** so hop SK 1.46’s canboatjs 1.27 can encode the JSON. Stock `@canboat/pgns` 1.4.2 and published `@canboat/ts-pgns` 1.11.18 have no Simnet 65332 classes.
...
- After ts-pgns publishes Simnet 65332, `lib/simnet-zc.json` can retire. SK 1.27 still needs `canboat-custom-pgns` until SK itself uses canboatjs 3.
