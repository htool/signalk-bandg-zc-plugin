...
- Emit NMEA as JSON: `app.emit('nmea2000JsonOut', pgn)` from `lib/zc-n2k.js`. Create 65332 with canboatjs 3 `toPgn` (canboat-derived defs). Also `canboat-custom-pgns` so SK 1.27 can encode. Do not emit HEX `nmea2000out`.
...
- Zeus/IE11 webapp: `var` / `function`, no optional chaining.
- Add tests in `test/` for that slice. `npm test` must pass.
- After a code change that boatnet runs, copy to `pi@boatnet:/home/pi/src/signalk-bandg-zc-plugin` and `sudo systemctl restart signalk`. Keep plugin config.
